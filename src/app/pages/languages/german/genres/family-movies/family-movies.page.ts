...
@Component({
  selector: 'app-family-movies',
  templateUrl: './family-movies.page.html',
  styleUrls: ['./family-movies.page.scss'],
})
export class FamilyMoviesPage implements OnInit {
  movies = [];
  genres = null;
  currentPage = 1; 
  ...
  ngOnInit() {
   ...
   async loadMovies(event?){
     ...
     this.movieDeService.getFamilyMoviesDe(this.currentPage).subscribe((res)=>{
       loading.dismiss();

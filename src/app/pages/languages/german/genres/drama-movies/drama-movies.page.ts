...
@Component({
  ...
  templateUrl: './drama-movies.page.html',
  styleUrls: ['./drama-movies.page.scss'],
})
export class DramaMoviesPage implements OnInit {
  movies = [];
  genres = null;
  currentPage = 1; 
  ...
  ngOnInit() {
   ...
   async loadMovies(event?){
     ...
     this.movieDeService.getDramaMoviesDe(this.currentPage).subscribe((res)=>{
       loading.dismiss();

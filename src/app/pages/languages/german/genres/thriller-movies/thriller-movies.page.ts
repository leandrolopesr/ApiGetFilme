...
@Component({
  ...
  templateUrl: './thriller-movies.page.html',
  styleUrls: ['./thriller-movies.page.scss'],
})
export class ThrillerMoviesPage implements OnInit {
  movies = [];
  genres = null;
  currentPage = 1; 
  ...
  ngOnInit() {
   ...
   async loadMovies(event?){
     ...
     this.movieDeService.getThrillerMoviesDe(this.currentPage).subscribe((res)=>{
       loading.dismiss();

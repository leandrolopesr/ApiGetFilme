...
@Component({
  ...
  templateUrl: './romance-movies.page.html',
  styleUrls: ['./romance-movies.page.scss'],
})
export class RomanceMoviesPage implements OnInit {
  movies = [];
  genres = null;
  currentPage = 1; 
  ...
  ngOnInit() {
   ...
   async loadMovies(event?){
     ...
     this.movieDeService.getRomanceMoviesDe(this.currentPage).subscribe((res)=>{
       loading.dismiss();

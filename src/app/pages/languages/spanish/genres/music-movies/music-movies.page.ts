...
@Component({
  selector: 'app-music-movies',
  templateUrl: './music-movies.page.html',
  styleUrls: ['./music-movies.page.scss'],
})
export class MusicMoviesPage implements OnInit {
  movies = [];
  genres = null;
  currentPage = 1; 
  ...
  ngOnInit() {
   ...
   async loadMovies(event?){
     ...
     this.movieEsService.getMusicMoviesEs(this.currentPage).subscribe((res)=>{
       loading.dismiss();

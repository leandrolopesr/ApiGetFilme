...
@Component({
  ...
  templateUrl: './documentary-movies.page.html',
  styleUrls: ['./documentary-movies.page.scss'],
})
export class DocumentaryMoviesPage implements OnInit {
  movies = [];
  genres = null;
  currentPage = 1; 
  ...
  ngOnInit() {
   ...
   async loadMovies(event?){
     ...
     this.movieEsService.getDocumentaryMoviesEs(this.currentPage).subscribe((res)=>{
       loading.dismiss();

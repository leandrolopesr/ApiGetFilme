...
@Component({
  selector: 'app-SF-movies',
  templateUrl: './SF-movies.page.html',
  styleUrls: ['./SF-movies.page.scss'],
})
export class SFMoviesPage implements OnInit {
  movies = [];
  genres = null;
  currentPage = 1; 
  ...
  ngOnInit() {
   ...
   async loadMovies(event?){
     ...
     this.movieEsService.getSFMoviesEs(this.currentPage).subscribe((res)=>{
       loading.dismiss();

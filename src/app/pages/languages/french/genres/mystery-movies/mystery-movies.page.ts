...
@Component({
  selector: 'app-mystery-movies',
  templateUrl: './mystery-movies.page.html',
  styleUrls: ['./mystery-movies.page.scss'],
})
export class MysteryMoviesPage implements OnInit {
  movies = [];
  genres = null;
  currentPage = 1; 
  ...
  ngOnInit() {
   ...
   async loadMovies(event?){
     ...
     this.movieFrService.getMysteryMoviesFr(this.currentPage).subscribe((res)=>{
       loading.dismiss();

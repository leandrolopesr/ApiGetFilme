...
@Component({
  ...
  templateUrl: './adventure-movies.page.html',
  styleUrls: ['./adventure-movies.page.scss'],
})
export class AdventureMoviesPage implements OnInit {
  movies = [];
  genres = null;
  currentPage = 1; 
  ...
  ngOnInit() {
   ...
   async loadMovies(event?){
     ...
     this.movieFrService.getAdventureMoviesFr(this.currentPage).subscribe((res)=>{
       loading.dismiss();

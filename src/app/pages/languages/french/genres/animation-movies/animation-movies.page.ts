...
@Component({
  ...
  templateUrl: './animation-movies.page.html',
  styleUrls: ['./animation-movies.page.scss'],
})
export class AnimationMoviesPage implements OnInit {
  movies = [];
  genres = null;
  currentPage = 1; 
  ...
  ngOnInit() {
   ...
   async loadMovies(event?){
     ...
     this.movieFrService.getAnimationMoviesFr(this.currentPage).subscribe((res)=>{
       loading.dismiss();

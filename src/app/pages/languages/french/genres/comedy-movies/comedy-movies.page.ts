...
@Component({
  ...
  templateUrl: './comedy-movies.page.html',
  styleUrls: ['./comedy-movies.page.scss'],
})
export class ComedyMoviesPage implements OnInit {
  movies = [];
  genres = null;
  currentPage = 1; 
  ...
  ngOnInit() {
   ...
   async loadMovies(event?){
     ...
     this.movieFrService.getComedyMoviesFr(this.currentPage).subscribe((res)=>{
       loading.dismiss();

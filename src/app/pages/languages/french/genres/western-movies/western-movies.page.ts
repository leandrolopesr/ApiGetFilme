...
@Component({
  selector: 'app-western-movies',
  templateUrl: './western-movies.page.html',
  styleUrls: ['./western-movies.page.scss'],
})
export class WesternMoviesPage implements OnInit {
  movies = [];
  genres = null;
  currentPage = 1; 
  ...
  ngOnInit() {
   ...
   async loadMovies(event?){
     ...
     this.movieFrService.getWesternMoviesFr(this.currentPage).subscribe((res)=>{
       loading.dismiss();
